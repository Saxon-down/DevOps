...
# New pre-receive hook for GitHub Enterprise, to replace the current
# (test, unsecured) script.
...
GKE_LOGSTASH_SERVER = "https://127.0.0.1:8080"
GKE_LOGSTASH_CA_CERT = "/usr/src/app/ca.cert"
GKE_LOGSTASH_SSL_CERT = "/usr/src/app/elk-gke.pem"  # path on docker image

POC_LOGSTASH_SERVER = "https://127.0.0.1:5011"
POC_LOGSTASH_SSL_CERT = "/usr/src/app/test-ssl.pem"
POC_LOGSTASH_CA_CERT = "/usr/src/app/test-ca.cert"
...
def post_to_logstash(json_block):
    ...
    with requests.session() as elk_session:
        ...
        elk_session = requests.post(logstash_server, data=json_block, cert=logstash_ssl_cert, verify=logstash_ca_cert)
